...
To determine the room dimensions, we display the data on \texttt{Rviz} and then use the selection tool determine the maximum and minimum value on every axis that defines the room dimensions. This process is manual, and prone to error.
...
\begin{table}[H]
	...
	\begin{tabular}{@{}llC{2cm}C{2cm}C{2cm}C{2cm}@{}}
		\toprule
		\multicolumn{2}{l}{Test Scenario} & \# \ac{lidar} messages & Average Points Per Message & \# interfered points & Relative \% of points \\
			\midrule
		\multicolumn{2}{l}{\textit{Velodyne VLP-16}}  & 1  & 2 & 3 & 4  \\ 
			\phantom{a} & Test \#1  & 1  & 2 & 3 & 4  \\ 
			\phantom{a} & Test \#2  & 1  & 2 & 3 & 4  \\ 
		\midrule
		\multicolumn{2}{l}{\textit{HESAI Pandar40}} & 1 & 2 & 3 & 4  \\ 
			\phantom{a} & Test \#1  & 1  & 2 & 3 & 4  \\ 
	\end{tabular}
...
For our experimental dataset, we can automate the determination of the room dimensions from the data, since we have measurements without interference. We developed a \ac{ros} node that is capable of parsing a ground truth bag file and determining the room dimensions, on the \ac{lidar} coordinate frame. To account for some noise on the data, the majorant and minorant of the room dimensions was considered and a 5\% tolerance was used. All the points that are not contained within the room dimensions of all three Euclidian coordinates plus the tolerance, are considered outliers.
