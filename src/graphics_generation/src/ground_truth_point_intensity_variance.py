...
if len(sys.argv) != 2 and len(sys.argv) != 3:
    sys.exit("""Wrong number of arguments!\n
                Usage: rosrun point_cloud_statistics ground_truth_point_intensity_variance.py <test scenario codename>
                Usage: rosrun point_cloud_statistics ground_truth_point_intensity_variance.py <test scenario codename> <number of bins>""")

graphics_folder = datasets_path.makeGraphicsDirectory(sys.argv[1], datasets_path.GRAPHICS_FOLDER_RELATIVE_PATH)
filename = datasets_path.constructFullPathToResults(sys.argv[1], datasets_path.GROUND_TRUTH_POINT_INTENSITY_VARIANCE_BIN_NAME)
...
data_label = "ground truth model intensity variance"
...
hist_filename = datasets_path.constructFullPathToGraphics(sys.argv[1], datasets_path.GROUND_TRUTH_POINT_INTENSITY_VARIANCE_HIST_FILE_NAME)
...
colored_mesh_filename = datasets_path.constructFullPathToGraphics(sys.argv[1], datasets_path.GROUND_TRUTH_POINT_INTENSITY_VARIANCE_COLOR_MESH_FILE_NAME)
