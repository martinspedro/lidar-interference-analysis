...
filename = constructFullPathToResults(sys.argv[1], INTERFERENCE_BAG_POINTS_INTENSITY_VECTOR_BIN_NAME)
...
data_label = "interference intensity difference"
...
n, bins, patches = ax.hist(data_clean, number_of_bins, density=True, histtype='bar', align='mid', log=False, label=data_label)
#ax.legend()
ax.set_xlabel('Intensity difference')
ax.set_ylabel('Normalized probability density')
ax.set_title("Intensity difference between interference bag and ground_truth_model")
fig1.tight_layout()
plt.show(block=False)

print("Done! Saving histogram figure... ")

hist_filename = constructFullPathToGraphics(sys.argv[1], INTERFERENCE_BAG_INTENSITY_HIST_FILE_NAME)
