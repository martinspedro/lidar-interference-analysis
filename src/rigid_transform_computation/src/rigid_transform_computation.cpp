...
int main(int argc, char* argv[])
{
    ...
    else
    {
        ...
        camera_info_sub = nh.subscribe<sensor_msgs::CameraInfo>("camera/camera_info", 10,
                                            &LiDARCameraCalibrationData::cameraInfoCallback, &calibration_data_object);
        std::cout << "Done!" << std::endl;
    }
    ...
    ros::ServiceServer computationSrv = nh.advertiseService("computeRigidBodyTransform", &LiDARCameraCalibrationData::computeRigidTransformSrvCallback, &calibration_data_object);
    ros::ServiceServer saveSrv        = nh.advertiseService("save_correspondences", &LiDARCameraCalibrationData::saveCorrespondencesSrvCallback, &calibration_data_object);
