...
PointCloudVisualizer::PointCloudVisualizer(std::string point_cloud_topic, std::string node_handler_name)
{
  ...
  this->point_cloud_sub = this->nh_->subscribe<sensor_msgs::PointCloud2>(point_cloud_topic, DEFAULT_QUEUE_SIZE,
                                                                         &PointCloudVisualizer::viewerCallback, this);

  this->pcl_viewer = pcl::visualization::PCLVisualizer::Ptr(new pcl::visualization::PCLVisualizer(
      POINT_CLOUD_VIEWER_NAME));  // new pcl::visualization::PCLVisualizer(POINT_CLOUD_VIEWER_NAME); //!< Create
...
PointCloudVisualizer::PointCloudVisualizer(std::string point_cloud_topic, std::string pose_topic,
                                           std::string node_handler_name)
{
  ...
  this->point_cloud_sub = this->nh_->subscribe<sensor_msgs::PointCloud2>(
      point_cloud_topic, DEFAULT_QUEUE_SIZE, &PointCloudVisualizer::viewerWithPoseCallback, this);
  this->pose_pub = this->nh_->advertise<geometry_msgs::Pose>(pose_topic, DEFAULT_POSE_QUEUE_SIZE);

  this->point_pub = this->nh_->advertise<rigid_transform_computation::PointXYZI>("picked_point", DEFAULT_QUEUE_SIZE);
...
void PointCloudVisualizer::initPointCloudVisualizer()
{
  ...
  this->pcl_viewer->setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 3,
                                                     "point_cloud_name");
}

void PointCloudVisualizer::addNewPointCloudToVisualizer(const sensor_msgs::PointCloud2::ConstPtr& point_cloud_msg,
                                               ...
                                               void* viewerVoidPtr)
{
  ...
  // PointCloudVisualizer::PointCloudVisualizer* aux_pcl_handler = (PointCloudVisualizer::PointCloudVisualizer*)
  // viewerVoidPtr;
  ...
  if (idx != -1)
  {
    ...
    std::cout << "Point Clicked at ( " << x << ", " << y << ", " << z << ")" << std::endl;
    ...
    clicked_points.push_back(pcl::PointXYZ(x, y, z));
  }
}
...
void PointCloudVisualizer::registerPointPickingCallback(const uint8_t MODE)
{
  ...
  {
    this->pcl_viewer->registerPointPickingCallback(PointCloudVisualizer::onPointPickingEvent,
                                                   (void*)&(this->point_pub));
  }
  else
  {
...
void PointCloudVisualizer::spin()
{
  ...
  {
    const std::string sphere_id = "sphere_" + std::to_string(i);
    if (!this->pcl_viewer->contains(sphere_id))
    {
      this->pcl_viewer->addSphere(clicked_points[i], 0.03d, 1.0d, 0.0d, 0.0d, sphere_id);
    }
  }
