...
void LiDARCameraCalibrationData::solvePnP(int solvePnP_mode)
{
  ...
  {
    case 0:
      cv::solvePnP(this->point_cloud_points_, this->image_pixels_, this->camera_info_.fullIntrinsicMatrix(),
                   this->camera_info_.distortionCoeffs(), this->rotation_vector_, this->translation_vector_, false,
...
void LiDARCameraCalibrationData::computeRigidBodyTransform(int pnp_mode)
{
  ...
  std::cout << "Normalized Camera to LiDAR Quaternion (x, y, z, w): (" << this->rotation_quaternion_.x() << ", "
            << this->rotation_quaternion_.y() << ", " << this->rotation_quaternion_.z() << ", "
            << this->rotation_quaternion_.w() << ")" << std::endl;
  std::cout << "Normalized LiDAR to Camera Quaternion (x, y, z, w): (" << this->rotation_quaternion_.x() << ", "
            << this->rotation_quaternion_.y() << ", " << this->rotation_quaternion_.z() << ", "
            << -this->rotation_quaternion_.w() << ")" << std::endl;
}
...
void LiDARCameraCalibrationData::readCameraInfoFromYAML(std::string filename)
{
  ...
  // Instead of creating YAML nodes to deal with Map events, acess directly the data field, since the others are not
  // needed
...
bool LiDARCameraCalibrationData::saveCorrespondencesSrvCallback(
    rigid_transform_computation::save_correspondences::Request& req,
...
bool LiDARCameraCalibrationData::computeRigidTransformSrvCallback(
    rigid_transform_computation::compute_rigid_body_transform::Request& req,
    rigid_transform_computation::compute_rigid_body_transform::Response& res)
{
  this->computeRigidBodyTransform(req.solve_PnP_algorithm);
  res.rigid_body_transform = this->createStaticTransformMsg();
