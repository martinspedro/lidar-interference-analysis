...
ImageVisualizer::ImageVisualizer(std::string camera_root_topic, std::string clicked_pixel_topic) : it_(nh_)
{
  ...
  this->image_sub_ = this->it_.subscribe(this->camera_root_topic + this->image->getImageColorTopic(),
                                         DEFAULT_QUEUE_SIZE, &ImageVisualizer::viewerCallback, this);
  ...
  this->pixel_pub =
      this->nh_.advertise<rigid_transform_computation::Pixel>(clicked_pixel_topic, DEFAULT_PIXEL_QUEUE_SIZE);
  ...
  : it_(nh_)
{
  ...
  this->image = new Image();
  // Subscribe to input video feed and publish output video feed
  this->image_sub_ = this->it_.subscribe(camera_root_topic + this->image->getImageColorTopic(), DEFAULT_QUEUE_SIZE,
...
void ImageVisualizer::viewerCallback(const sensor_msgs::ImageConstPtr& msg)
{
  try
  {
    ...
    for (int i = 0; i < clicked_pixels.size(); ++i)
    {
      cv::drawMarker(this->cv_ptr->image, clicked_pixels[i], cv::Scalar(0, 0, 255), MARKER_TILTED_CROSS, 15, 4, 8);
    }
...
void ImageVisualizer::onMouse(int event, int x, int y, int flags, void* param)
{
  ...
  if (event == cv::EVENT_LBUTTONDOWN)
  {
    ...
    if (tempMouseCallback.imgPtr)
    {
      ...
      clicked_pixels.push_back(cv::Point2i(x, y));  // Save clicked points

      for (int i = 0; i < clicked_pixels.size(); ++i)
      {
        cv::drawMarker(imgPtr, clicked_pixels[i], cv::Scalar(0, 0, 255), MARKER_TILTED_CROSS, 15, 4, 8);
      }
      cv::imshow("Video Viewer", imgPtr);
      cv::imwrite("/home/martinspedro/aux.jpg", imgPtr);
    }
    else
    {
